...
export default function KdsPage() {
  ...
  return (
    <main className="bg-[#06080C] text-[#F4F0EA] min-h-screen font-sans p-4 sm:p-8">
      ...
      {/* STAFF KDS HEADER (PASSWORD FREE, NO FINANCIALS) */}
      ...
      {activeTab === "kds" ? (
        ...
        /* --- STAFF INVENTORY / STOCK CONTROL TAB --- */
        <div className="space-y-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              <h2 className="font-serif text-2xl text-white">Menu Stock Control</h2>
              <p className="text-xs text-gray-400 mt-1 uppercase tracking-widest">Mark items in or out of stock instantly</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredStaffMenu.map((item) => (
              <div key={item.id} className={`bg-[#12100E] border rounded-2xl p-6 shadow-xl flex flex-col justify-between ${item.is_available ? 'border-white/10' : 'border-red-500/30 opacity-60'}`}>
                <div>
                  ...
                  <p className="text-gray-400 text-xs leading-relaxed mb-6">₹{item.price} — {item.description}</p>
                </div>
